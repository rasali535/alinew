...
const generatorApplied = '  // Media Generators — Prompt-faithful server generation only.';
...
if (growth.includes(generatorStart)) {
  growth = replaceBetween(growth, generatorStart, generatorEnd, generatorReplacement, 'Growth media generator');
} else if (!growth.includes(generatorApplied)) {
  throw new Error('Growth media generator is neither original nor already patched.');
}

const qaStart = '                  {/* Commercial Visual QA Scorecard Widget */}';
const qaApplied = '                  {/* Commercial Visual Validation — only shown after a real server-generated asset succeeds. */}';
...
if (growth.includes(qaStart)) {
  growth = replaceBetween(growth, qaStart, qaEnd, qaReplacement, 'hard-coded creative QA widget');
} else if (!growth.includes(qaApplied)) {
  throw new Error('Creative QA widget is neither original nor already patched.');
}
