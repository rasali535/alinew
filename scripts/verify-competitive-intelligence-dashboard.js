...
for (const file of [panelPath, growthPath, apiPath]) {
...
assert(growth.includes('<CompetitiveIntelligencePanel'), 'Growth page must render CompetitiveIntelligencePanel');
assert(growth.includes('MARKET_INTEL'), 'Growth page must retain the Market Intelligence workspace tab');
...
assert(api.includes('publicSourcesOnly') || read('apps/ralion/src/lib/services/mari/mariCompetitiveIntelligence.service.ts').includes('publicSourcesOnly'), 'Competitive Intelligence must preserve public-source-only rules');
