...
async function monitorExecution() {
  const targetId = '6a84b330b6e4cdbded32e338';
  console.log(`\nMonitoring execution for scheduled post ID: ${targetId}...`);
  ...
  // Poll every 30 seconds for up to 8 minutes
  for (let attempt = 1; attempt <= 16; attempt++) {
    const now = new Date().toISOString();
    console.log(`\n[Attempt ${attempt}/16 at ${now}] Checking live post status...`);

    const postsRes = await request(`${PROD_URL}/api/social/facebook/pages/default/posts`, {
      headers: { Authorization: `Bearer ${tokenA}` },
    });

    const post = postsRes.data?.posts?.find(p => p.id === targetId || p.platformPostId === targetId || p.body?.includes('Autonomous Cloud Scheduled Post'));

    if (post) {
      console.log(`Post Status: [${post.status.toUpperCase()}]`, {
        id: post.id,
      ...
      if (post.status === 'published') {
        console.log('\n🎉 SUCCESS: Scheduled post has successfully published to Facebook Page!');
        console.log(`Facebook Permalink: ${post.permalink || `https://www.facebook.com/${post.platformPostId || post.id}`}`);
        return post;
      }
    } else {
      console.log('Post not yet returned in feed response.');
    }
