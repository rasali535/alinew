      ...
      <div className="aspect-[4/3] overflow-hidden rounded-lg bg-white/5">
        <img
          ...
          onError={(e) => {
            ...
            e.target.className = 'w-full h-full object-cover opacity-60 contrast-125';
          }}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      </div>
      <div className="mt-4">
        <h3 className="text-white text-xl md:text-2xl font-light group-hover:text-brand-gold transition-colors duration-300">
          {project.title}
        </h3>
        ...
        {project.link && (
          <span className="inline-block mt-2 text-xs text-brand-gold border-b border-brand-gold pb-0.5">
            Visit Website
          </span>
        )}
      </div>
