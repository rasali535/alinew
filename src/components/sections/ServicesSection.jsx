...
const ServiceCard = ({ service, isExpanded, onToggle }) => (
  ...
  >
    <div className="px-6 lg:px-12 flex items-center justify-between">
      <h3 className="text-white text-2xl md:text-3xl lg:text-4xl font-light group-hover:text-brand-gold transition-colors duration-300">
        {service.title}
      </h3>
      <ChevronRight
    ...
    >
      <div className="px-6 lg:px-12 grid grid-cols-2 md:grid-cols-4 gap-4">
        {service.items.map((item, index) => (
          <span
            key={index}
            className="text-white/60 text-sm hover:text-brand-gold transition-colors duration-300 cursor-pointer"
          >
            {item}
          </span>
        ))}
      ...
      <div className="px-6 lg:px-12 mt-8 mb-4">
        <Link
          ...
          className="inline-block bg-brand-gold text-black px-6 py-2 rounded-full text-sm font-medium hover:bg-white transition-colors duration-300"
        >
          Book {service.title}
        </Link>
      </div>
