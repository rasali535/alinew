...
type Tab = 'overview' | 'customers' | 'social' | 'billing' | 'system' | 'audit';
...
const statusStyle = (status: string) => {
  if (status === 'UP' || status === 'ACTIVE' || status === 'CONNECTED' || status === 'SUCCESS') return 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300';
  if (status === 'DEGRADED' || status === 'PAST_DUE') return 'border-amber-500/30 bg-amber-500/10 text-amber-300';
      ...
      <main className="max-w-7xl mx-auto p-5 space-y-5">
        ...
        {tab === 'overview' && metrics && <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <InfoCard title="Social Connections" value={metrics.activeSocialConnections || 0} detail={`Meta ${metrics.connectedMetaAccounts || 0} · Zernio ${metrics.connectedZernioProfiles || 0}`} tone="good" />
        ...
        {tab === 'customers' && <section className="space-y-4"><div className="flex flex-col md:flex-row gap-3 md:items-center justify-between"><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search tenant, owner, email or ID…" className="w-full md:max-w-md px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-800 text-xs" /><div className="text-xs text-zinc-500">{filteredCustomers.length} visible tenants</div></div><div className="overflow-x-auto rounded-2xl border border-zinc-800"><table className="w-full text-xs"><thead className="bg-zinc-900 text-zinc-400"><tr><th className="p-3 text-left">Tenant</th><th className="p-3 text-left">Plan</th><th className="p-3 text-left">Credits</th><th className="p-3 text-left">Knowledge</th><th className="p-3 text-left">Social</th><th className="p-3 text-left">Status</th><th className="p-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-zinc-800">{filteredCustomers.map(customer => <tr key={customer.organizationId} className="bg-zinc-950/40"><td className="p-3"><div className="font-semibold text-white">{customer.name}</div><div className="font-mono text-[10px] text-zinc-600">{customer.organizationId}</div><div className="text-[10px] text-zinc-500">{customer.ownerEmail || 'No owner email'}</div></td><td className="p-3">{customer.plan}</td><td className="p-3 font-mono">{customer.credits || 0}<div className="text-[10px] text-zinc-600">{customer.creditsConsumed || 0} consumed</div></td><td className="p-3"><Pill text={customer.websiteIngestionStatus || 'NONE'} status={customer.websiteIngestionStatus === 'VERIFIED' ? 'ACTIVE' : 'UNKNOWN'} /></td><td className="p-3 space-y-1"><Pill text={`Meta ${customer.facebookStatus || customer.metaStatus || 'DISCONNECTED'}`} status={customer.facebookStatus || customer.metaStatus || 'DISCONNECTED'} /> <Pill text={`Zernio ${customer.zernioStatus || 'DISCONNECTED'}`} status={customer.zernioStatus || 'DISCONNECTED'} /></td><td className="p-3"><Pill text={customer.status} status={customer.status} /></td><td className="p-3"><div className="flex justify-end gap-2"><button onClick={() => void inspectTenant(customer.organizationId)} className="p-2 rounded-lg border border-zinc-700 bg-zinc-900" title="Inspect"><Eye className="w-3.5 h-3.5" /></button><button onClick={() => setCreditOrg(customer.organizationId)} className="p-2 rounded-lg border border-indigo-500/30 bg-indigo-500/10 text-indigo-300" title="Adjust credits"><Coins className="w-3.5 h-3.5" /></button><button onClick={() => void updateTenantStatus(customer)} className={`px-2 py-1 rounded-lg border text-[10px] font-bold ${customer.status === 'ACTIVE' ? 'border-rose-500/30 text-rose-300' : 'border-emerald-500/30 text-emerald-300'}`}>{customer.status === 'ACTIVE' ? 'Suspend' : 'Reactivate'}</button></div></td></tr>)}{!filteredCustomers.length && <tr><td colSpan={7} className="p-10 text-center text-zinc-500">No matching organizations.</td></tr>}</tbody></table></div><div className="flex items-center gap-2"><input value={inspectReason} onChange={e => setInspectReason(e.target.value)} className="flex-1 max-w-lg px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-800 text-xs" placeholder="Reason recorded when inspecting a tenant" /><span className="text-[10px] text-zinc-600">Inspection is audit logged.</span></div></section>}

        {tab === 'social' && metrics && <section className="space-y-4"><div className="grid grid-cols-3 gap-4"><Metric title="Active Bindings" value={metrics.activeSocialConnections || 0} detail="Canonical active social connections" icon={Share2} /><Metric title="Meta" value={metrics.connectedMetaAccounts || 0} detail="Active Facebook/Meta bindings" icon={Share2} /><Metric title="Zernio" value={metrics.connectedZernioProfiles || 0} detail="Active provider profiles" icon={Share2} /></div><div className="grid md:grid-cols-2 gap-3">{(metrics.allConnections || []).map(conn => <div key={conn.id} className="p-4 rounded-xl border border-zinc-800 bg-zinc-900/50"><div className="flex justify-between gap-3"><div><div className="font-semibold text-sm">{conn.accountName}</div><div className="text-[10px] text-zinc-500">{conn.provider} · {conn.accountTypeLabel || conn.accountType || 'Account'} · {conn.providerAccountId}</div></div><Pill text={conn.connectionStatus} status={conn.connectionStatus} /></div><div className="text-[10px] text-zinc-600 mt-2">Tenant: {conn.organizationId || 'unbound'} · {Number(conn.followersCount || 0).toLocaleString()} followers</div></div>)}{!metrics.allConnections?.length && <div className="text-xs text-zinc-500">No active social connections.</div>}</div></section>}

        {tab === 'billing' && <section className="space-y-4"><div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><Metric title="Estimated MRR" value={`$${Number(metrics?.estimatedMRR || 0).toLocaleString()}`} detail="Active USD-priced subscriptions" icon={TrendingUp} /><Metric title="Credits Issued" value={Number(metrics?.totalCreditsIssued || 0).toLocaleString()} detail="Lifetime durable wallet grants" icon={Coins} /><Metric title="Credits Consumed" value={Number(metrics?.totalCreditsConsumed || 0).toLocaleString()} detail="Lifetime durable consumption" icon={Coins} /><Metric title="Paying Tenants" value={metrics?.payingCustomers || 0} detail="Active plans with non-zero price" icon={CreditCard} /></div><div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4 text-xs text-zinc-400"><strong className="text-zinc-200">Accounting note:</strong> MRR excludes non-USD prices rather than inventing an exchange rate. Credit totals come directly from durable tenant wallets.</div></section>}
