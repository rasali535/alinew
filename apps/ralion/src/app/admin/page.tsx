...
const statusStyle = (status: string) => {
  if (status === 'UP' || status === 'ACTIVE' || status === 'CONNECTED' || status === 'SUCCESS' || status === 'LIVE') return 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300';
  if (status === 'DEGRADED' || status === 'PAST_DUE' || status === 'CONFIGURED' || status === 'IDLE' || status === 'PAUSED') return 'border-amber-500/30 bg-amber-500/10 text-amber-300';
