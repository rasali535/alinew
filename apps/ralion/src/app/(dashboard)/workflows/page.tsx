...
type TriggerEvent = 'CUSTOMER_CREATED' | 'DEAL_STAGE_CHANGED' | 'TASK_COMPLETED' | 'SOCIAL_COMMENT_RECEIVED' | 'SOCIAL_INBOX_RECEIVED' | 'MANUAL';
type ActionType = 'CREATE_TASK' | 'CREATE_CALENDAR_EVENT' | 'AUDIT_LOG' | 'MARI_DECISION' | 'REPLY_SOCIAL_COMMENT' | 'REPLY_SOCIAL_INBOX';
...
export default function WorkflowsPage() {
  ...
  const createWorkflow = async () => {
    ...
    if (form.actionType === 'MARI_DECISION') config.autoApproveLowRisk = false;
    if (form.actionType === 'REPLY_SOCIAL_COMMENT') config.replyText = form.actionTitle.trim();
    if (form.actionType === 'REPLY_SOCIAL_INBOX') config.messageText = form.actionTitle.trim();
  ...
  return (
    <TierAccessGate requiredTier="STANDARD" featureName="No-Code Automated Workflows" description="Automate customer, CRM and task events with bounded server-side actions.">
      <div className="flex flex-col gap-6 max-w-7xl mx-auto">
        ...
        <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Build Workflow"><div className="flex flex-col gap-4"><input value={form.name} onChange={e => setForm(v => ({ ...v, name: e.target.value }))} placeholder="Workflow name" className="px-3 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-xs text-white" /><select value={form.triggerEvent} onChange={e => setForm(v => ({ ...v, triggerEvent: e.target.value as TriggerEvent }))} className="px-3 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-xs text-white"><option value="CUSTOMER_CREATED">Customer created</option><option value="DEAL_STAGE_CHANGED">Deal stage changed</option><option value="TASK_COMPLETED">Task completed</option><option value="SOCIAL_COMMENT_RECEIVED">New social comment</option><option value="SOCIAL_INBOX_RECEIVED">New inbox message</option><option value="MANUAL">Manual only</option></select><select value={form.actionType} onChange={e => setForm(v => ({ ...v, actionType: e.target.value as ActionType }))} className="px-3 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-xs text-white"><option value="CREATE_TASK">Create task</option><option value="CREATE_CALENDAR_EVENT">Create calendar reminder</option><option value="AUDIT_LOG">Write audit event</option><option value="MARI_DECISION">Ask Mari to understand & decide</option><option value="REPLY_SOCIAL_COMMENT">Reply to social comment</option><option value="REPLY_SOCIAL_INBOX">Reply to inbox message</option></select><input value={form.actionTitle} onChange={e => setForm(v => ({ ...v, actionTitle: e.target.value }))} placeholder={form.actionType === 'AUDIT_LOG' ? 'Audit note' : form.actionType === 'MARI_DECISION' ? 'Optional decision instruction' : form.actionType.startsWith('REPLY_SOCIAL') ? 'Reply text' : 'Task / event title'} className="px-3 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-xs text-white" />{form.actionType === 'CREATE_TASK' && <input type="number" min="0" max="365" value={form.dueDays} onChange={e => setForm(v => ({ ...v, dueDays: e.target.value }))} placeholder="Due in days" className="px-3 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-xs text-white" />}<div className="rounded-lg bg-zinc-950 border border-zinc-800 p-3 text-[10px] text-zinc-400">Workflows are tenant-scoped and audited. Social replies use the connected account for this workspace; live delivery requires the relevant provider permissions and connection.</div><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => setModalOpen(false)}>Cancel</Button><Button variant="primary" size="sm" onClick={() => void createWorkflow()}>Save Workflow</Button></div></div></Modal>
      </div>
    </TierAccessGate>
  );
}
