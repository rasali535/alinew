...
function authErrorStatus(code?: string): number {
  ...
  if (
    ...
    || code === 'MARI_API_REQUEST_LIMIT_REACHED'
    || code === 'MARI_API_CREDIT_LIMIT_REACHED'
  ) return 429;
  return 401;
}
...
async function recordRejectedRequest(params: {
  authContext: NonNullable<Awaited<ReturnType<typeof MariApiKeyService.authenticate>>>;
  requestId: string;
  statusCode: number;
  startedAt: number;
}) {
  await MariApiKeyService.recordUsage({
    apiKeyId: params.authContext.id,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!query) {
      await recordRejectedRequest({ authContext, requestId, statusCode: 400, startedAt });
    ...
    if (query.length > 12000) {
      await recordRejectedRequest({ authContext, requestId, statusCode: 413, startedAt });
