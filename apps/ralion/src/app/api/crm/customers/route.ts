...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('customers').select('*').eq('workspace_id', workspaceId).order('created_at', { ascending: false });
    ...
    return corsJsonResponse({ success: false, error: 'Failed to load customers.' }, { status: 500 }, request);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !email) return corsJsonResponse({ success: false, error: 'Customer name and email are required.' }, { status: 400 }, request);
    ...
    await writeOperationalAudit(
      ...
      'CUSTOMER_CREATED',
      'CRM',
      { customerId: data.id, name: data.name, email: data.email }
    );
    try {
      ...
      console.warn('[CRM Customers API] workflow trigger failed:', workflowError?.message);
    }
    ...
    return corsJsonResponse({ success: false, error: 'Failed to create customer.' }, { status: 500 }, request);
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const supabase = getServiceSupabase();
    const { data, error } = await supabase.from('customers').update(patch).eq('id', id).eq('workspace_id', workspaceId).select('*').maybeSingle();
    if (error) throw new Error(error.message);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) return corsJsonResponse({ success: false, error: 'Customer id is required.' }, { status: 400 }, request);
    const supabase = getServiceSupabase();
    const { data, error } = await supabase.from('customers').delete().eq('id', id).eq('workspace_id', workspaceId).select('id,name').maybeSingle();
    if (error) throw new Error(error.message);
    if (!data) return corsJsonResponse({ success: false, error: 'Customer not found.' }, { status: 404 }, request);
    await writeOperationalAudit(
      { organizationId: ctx.organization?.id, workspaceId, userId: ctx.user.id },
      'CUSTOMER_DELETED',
      'CRM',
      { customerId: data.id, name: data.name }
    );
