...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!contactName) {
      return corsJsonResponse({ success: false, error: 'Contact name is required.' }, { status: 400 }, request);
    }
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        assigned_to: cleanText(body.assignedTo, 160) || serverCtx.profile.fullName || serverCtx.user.email,
