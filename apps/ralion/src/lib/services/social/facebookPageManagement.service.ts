...
function getServiceSupabase() {
  ...
  if (!url) {
    throw new Error('SUPABASE_URL is not configured for Facebook Page management.');
  }
  if (!key) {
    throw new Error('SUPABASE_SERVICE_ROLE_KEY is not configured for Facebook Page management.');
  }
  ...
  static async connectPage(params: {
    ...
    try {
      ...
      const pageMeta = {
        ...
        tenantId: params.organizationId || params.workspaceId || 'default-tenant',
        workspaceId: params.workspaceId || params.organizationId || 'default-workspace',
        organizationId: params.organizationId || params.workspaceId || 'default-org',
        facebookUserId: satRow?.metadata?.facebookUserId || (allUserConns?.[0]?.metadata?.facebookUserId) || params.userId,
