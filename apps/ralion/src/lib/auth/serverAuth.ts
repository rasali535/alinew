...
export async function getCurrentRalionContext(
  ...
  // Fail closed if no persisted workspace can be proven for the authenticated user.
  // Workspace provisioning belongs in onboarding, not in an auth-context GET request.
  ...
  const membershipRole: RalionWorkspaceMembership['role'] =
    rawRole === 'owner' || rawRole === 'admin' || rawRole === 'member' || rawRole === 'viewer'
      ? rawRole
      : 'viewer';
